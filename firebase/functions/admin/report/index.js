...
const report = functions.database.ref('/admin/report/{pushId}').onCreate((event) => {
	...
	return Promise.all([profilePromise, stylendarPromise]).then((snapshots) => {
...
function sendReportEmail(report) {
	...
	text += 'Kill this reported user with fire to by tapping the link below. Be careful though, with power comes reponsibility!\n';
