...
const fixer = functions.https.onRequest((req, res) => {
	...
	var output = '';
	Promise.all([followingPromise, followersPromise]).then((snapshots) => {
		let promises = [], deletes = [];
		var followingPairsKeys = [], followerPairsKeys = [];
		...
		snapshots[0].forEach((child) => {
			...
			Object.keys(following).forEach((key) => {
				...
				if (!followingFrequency[user.uid]) {
					...
					promises.push(database.ref(`/veins/followers/${user.uid}`).orderByChild('uid').equalTo(`${child.key}`).once('value'));
				} else {
		...
		snapshots[1].forEach((child) => {
			...
			Object.keys(follower).forEach((key) => {
				...
				if (!followersFrequency[user.uid]) {
					...
					promises.push(database.ref(`/veins/following/${user.uid}`).orderByChild('uid').equalTo(`${child.key}`).once('value'));
				} else {
					const update = { [key]: null };
		...
		return Promise.all(promises).then((snapshots) => {
			...
			if (deletes.length) {
				return Promise.all(deletes).then(() => {
					res.status(200).send(`The cron job ran successfully, with ${deletes.length} fixes performed. The output is: <br>${output}`);
				});
			} else {
				res.status(200).send(`The cron job ran successfully, but no operations were performed. The output is: <br>${output}`);
			}
			return;
		});
