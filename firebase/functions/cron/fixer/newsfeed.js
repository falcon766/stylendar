...
const newsfeed = functions.https.onRequest((req, res) => {
	...
	Promise.resolve(stylendarPromise).then((snapshot) => {
		let stylendars = [];
		...
		return Promise.resolve(followersPromise).then((snapshot) => {
			...
			snapshot.forEach((child) => {
				...
				Object.keys(followers).forEach((key) => {
					...
					if (!follower || !follower.uid || !stylendar) { return; }
			...
			return Promise.all(promises).then((snapshots) => {
				...
				let promises = [], capsules = [];
				snapshots.forEach((snapshot) => {
					if (snapshot.exists() || snapshot.ref.key.includes('-')) { return; }
					...
					promises.push(database.ref(`/veins/stylendar/${capsule.uid}/${capsule.date.year}/${capsule.date.month}/${capsule.date.day}`).once('value'));
				...
				return Promise.all(promises).then((snapshots) => {
					...
					let insertions = []; const length = capsules.length * 4;
					for (var i = 0; i < length; i += 4) {
						...
						if (!capsule || !username || !name || !profileImageUrl || !imageUrl) { continue; }
						...
						const insertionPromise = database.ref(`/veins/newsfeed/${capsule.followerUid}/${capsule.pushId}`).set(update);
