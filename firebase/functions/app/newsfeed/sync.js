  ...
  Object.keys(stylendar).forEach(year => {
    ...
    Object.keys(yearObj).forEach(month => {
      ...
      Object.keys(monthObj).forEach(day => {
        ...
        if (!update.date || !update.imageUrl) {
          return;
        }
...
const sync = functions.database.ref("/veins/followers/{uid}/{pushId}").onWrite((change, context) => {
  ...
  const existencePromise = database
    .ref(`/veins/followers/${context.params.uid}/${context.params.pushId}`)
    .orderByChild("uid")
    .equalTo(change.before.val() ? change.before.val().uid : 0)
    .once("value");

  return Promise.all([profilePromise, stylendarPromise, existencePromise])
    .then(snapshots => {
      if (!snapshots[0].val() || !snapshots[1].val()) {
        return Promise.reject(
          "either the profile or the stylendar doesn't exist, so there aren't any modifications to make at all",
        );
      }
